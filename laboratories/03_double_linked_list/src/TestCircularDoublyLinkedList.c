...
int main() {
  struct CircularDoublyLinkedList *list, *first, *second, *res;
  ...
  first = malloc(sizeof(struct CircularDoublyLinkedList));
  second = malloc(sizeof(struct CircularDoublyLinkedList));
  ...
  add_nth_node(first, 0, &numbers[1]);
  add_nth_node(first, 1, &numbers[2]);
  add_nth_node(first, 2, &numbers[5]);
  add_nth_node(first, 3, &numbers[9]);

  add_nth_node(second, 0, &numbers[2]);
  add_nth_node(second, 1, &numbers[3]);
  add_nth_node(second, 2, &numbers[7]);
  add_nth_node(second, 3, &numbers[8]);
  add_nth_node(second, 4, &numbers[10]);

  res = merge_sorted_lists(first, second, compare_int);
  printf("#14:\n");
  printf("Output: ");
  print_int_list(res);
  printf("Output asteptat: 1 2 2 3 5 7 8 9 10\n");
  free_list(&first);
  free_list(&second);
  free_list(&res);
