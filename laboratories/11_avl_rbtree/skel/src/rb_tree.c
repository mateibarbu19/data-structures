 ...
 */
static rb_node_t *rb_node_create(void *key, void *data, size_t key_size,
                                 size_t data_size) {
  rb_node_t *rb_node;
...
rb_tree_t *rb_tree_create(int (*f)(const void *, const void *), size_t ksz,
                          size_t dsz) {
  rb_tree_t *rb_tree;
...
static void __swap_color(rb_node_t *node_a, rb_node_t *node_b) {
  ...
  tmp = (rb_node_t *) malloc(sizeof(rb_node_t));
  if (!tmp) {
    return;
  }
  tmp->color = node_a->color;
  node_a->color = node_b->color;
  node_b->color = tmp->color;
  free(tmp);
}
...
static int __check_violation(rb_tree_t *rb_tree, rb_node_t *rb_node) {
  return ((rb_node != rb_tree->root) && (rb_node->color != BLACK) &&
...
rb_node_t *rb_tree_insert(rb_tree_t *rb_tree, void *key, void *data) {
  ...
  /* procedam la fel ca la ABC, cautand pozitia unde sa inseram */
...
static void __rb_tree_free(rb_node_t *root, void (*f_key)(void *),
                           void (*f_data)(void *)) {
  if (root == NULL)
    return;
...
void rb_tree_free(rb_tree_t *rb_tree, void (*f_key)(void *),
                  void (*f_data)(void *)) {
